  ...
  if (items.length === 0 && !orderSuccess) {
    return (
      <div className="text-center py-20 animate-fade-in">
        ...
        <p className="text-gray-500 mb-6">Agrega algunos productos antes de continuar.</p>
        <a
          href="/catalogo"
  ...
  if (orderSuccess) {
    return (
      <div className="text-center py-20 animate-fade-in max-w-md mx-auto">
        ...
        {orderSuccessId && (
          <p className="text-indigo-600 font-bold mb-2">#ORD-{String(orderSuccessId).padStart(3, "0")}</p>
        )}
        <p className="text-gray-500 mb-8">
          Tu orden ha sido procesada correctamente. Te redirigiremos a tu cuenta en unos segundos.
        </p>
        <a
          href="/cuenta"
    ...
    <div className="flex flex-col lg:flex-row gap-10 items-start animate-fade-in">
      <div className="flex-1 w-full space-y-8">
        ...
        <button
          ...
          className={`w-full py-4 rounded-xl font-bold transition shadow-lg flex items-center justify-center gap-2 active:scale-[0.99] disabled:opacity-50 disabled:cursor-not-allowed ${
            metodoPago === "stripe"
              ? "text-lg bg-indigo-600 hover:bg-indigo-700 text-white"
              : "text-base sm:text-lg bg-[#009EE3] hover:bg-[#0088cc] text-white"
          }`}
        >
          {loadingOrder ? (
            <>
        ...
        <p className="text-xs text-center text-gray-400">
          Al confirmar tu compra, aceptas nuestros Términos y Condiciones y Política de Privacidad.
        </p>
      </div>

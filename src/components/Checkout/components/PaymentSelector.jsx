...
function StripePanel({ cardName, setCardName }) {
  ...
  return (
    <div className="bg-gray-50 border border-gray-200 rounded-xl p-5 space-y-4 animate-fade-in">
      ...
      {/* Número de tarjeta */}
      <div>
        <label className="text-xs font-medium text-gray-600 block mb-1">Número de tarjeta</label>
      ...
      <div>
        <label className="text-xs font-medium text-gray-600 block mb-1">Nombre en la tarjeta</label>
        <input
          id="stripe-card-name"
          type="text"
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-blue-700 mt-1">
            Serás redirigido al checkout seguro de Mercado Pago donde podrás pagar con tarjeta, transferencia bancaria,
            efectivo (OXXO, etc.) o tu saldo de cuenta.
          </p>
    ...
    <section>
      ...
      <div className="grid grid-cols-2 gap-3 mb-5">
        ...
        >
          <div
            className={`w-4 h-4 rounded-full border-2 flex-shrink-0 flex items-center justify-center ${
              metodoPago === "stripe" ? "border-indigo-600" : "border-gray-300"
            }`}
          >
            {metodoPago === "stripe" && <div className="w-2 h-2 bg-indigo-600 rounded-full" />}
          </div>
          <StripeLogo />
        </button>
        ...
        >
          <div
            className={`w-4 h-4 rounded-full border-2 flex-shrink-0 flex items-center justify-center ${
              metodoPago === "mercadopago" ? "border-blue-600" : "border-gray-300"
            }`}
          >
            {metodoPago === "mercadopago" && <div className="w-2 h-2 bg-blue-600 rounded-full" />}
          </div>
          <MPLogo />
        </button>
      </div>
